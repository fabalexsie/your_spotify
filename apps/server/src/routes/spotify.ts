...
router.post("/addtoqueue", logged, withHttpClient, async (req, res) => {
  const { client } = req as SpotifyRequest;
  const { id } = validate(req.body, addToQueueSchema);
...
router.get("/publication_date_per", isLoggedOrGuest, async (req, res) => {
  const { user } = req as LoggedRequest;
  const { start, end, timeSplit } = validate(req.query, intervalPerSchema);
...
router.get("/publication_date", isLoggedOrGuest, async (req, res) => {
  const { user } = req as LoggedRequest;
  const { start, end } = validate(req.query, interval);
...
router.get("/top/genres", isLoggedOrGuest, async (req, res) => {
  const { user } = req as LoggedRequest;
  ...
  const result = await getBestGenresNbOffseted(user, start, end, nb, offset);
  res.status(200).send(result);
});
